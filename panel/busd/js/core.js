...
const contractAddress="0x5D93f0CDf122bb09899747f484BcD39C6C1BFFD1"
const busdAddress="0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56";let contract2
let user={address:void 0,balance:void 0,balance_bnb:void 0,allowance:0,referrer:zeroAddress}
function setUpContracts(){mainContract=new web3.eth.Contract(abi,contractAddress)
if(!mainContract)return void 0
BUSDTokenContract=new web3.eth.Contract(BUSD_abi,busdAddress);contractLoaded()
console.log("Contract Loaded")
setUpAccount()}
window.addEventListener('load',async()=>{web3=new Web3(new Web3.providers.HttpProvider("https://data-seed-prebsc-1-s1.binance.org:8545"));web3.eth.setProvider(Web3.givenProvider);let accs=await window.ethereum.request({method:'eth_requestAccounts'});user.address=accs[0]
setUpContracts()
console.log("conn",accs[0])
try{await ethereum.enable();}catch(error){console.log('user rejected permission');}})
...
mainContract.methods.lottery_Pool().call({shouldPollResponse:true}).then(res=>{$('.avrch-1')[0].value=(res/1e18).toFixed(3)})
mainContract.methods.lottery_topBuy_today().call({shouldPollResponse:true}).then(res=>{$('.avrch-3')[0].value=(res/1e18).toFixed(3)})
mainContract.methods.lottery_topBuy_latest().call({shouldPollResponse:true}).then(res=>{$('.avrch-2')[0].value=(res/1e18).toFixed(3)})}
...
async function f(){BUSDTokenContract.methods.balanceOf(user.address).call({shouldPollResponse:false}).then(res=>{user.balance_bnb=parseInt(res)
setUserData()})}
f()})}
...
const serverUrl="https://q8ppkco2hirv.usemoralis.com:2053/server";const appId="CdQpQRRaH7Fdk5GSnLwqkva9Vms3j91mU84gN3zI";Moralis.start({serverUrl,appId});async function getMoralisData(){const data=await Moralis.Cloud.run("getEvents")
data[4].splice(15,data[3].length)
data[3].splice(15,data[4].length)
moralisRecentEvents=data
renderMoralisData(moralisRecentEvents,false)}
setInterval(()=>{getMoralisData()},1000*10)
getMoralisData()
let type_1_A="#267579"
let type_2_A="#4f6b78"
let type_3_A="#345f81"
...
let color_A,color_B
let txt
if(data[0]){data[0].forEach(el=>{txt=`Stake Started: ${((parseInt(el.attributes.rawAmount)).toFixed(2)/1e18).toFixed(0)} AVC for ${el.duration} days`
color_A=type_1_A
color_B=type_1_B
dores(el)})}
if(data[1]){data[1].forEach(el=>{txt=`Stake Collected: ${(parseInt(el.attributes.rawAmount)/1e18).toFixed(5)} BUSD`
color_A=type_1_A
color_B=type_1_B
dores(el)})}
if(data[2]){data[2].forEach(el=>{txt=`Auction Entered:<br> ${parseInt(el.attributes.rawAmount)/1e18} BUSD`
color_A=type_4_A
color_B=type_4_B
dores(el)})}
if(data[3]){data[3].forEach(el=>{counter++
if(counter<15){txt=`${(parseInt(el.attributes.rawAmount)/1e18).toFixed(0)} AVC Stake sell offer for ${parseInt(el.attributes.price)/1e18} BUSD`
color_A=type_3_A
color_B=type_3_B
dores(el)}})}
if(data[4]){data[4].forEach(el=>{txt=`${parseInt(el.attributes.rawAmount)/1e18} BUSD Loan request for ${parseInt(el.attributes.duration)} Days`
color_A=type_2_A
color_B=type_2_B
dores(el)})}
function dores(el){let p22=el.attributes.addr.slice(42-5)
$('.recent-events')[0].innerHTML+=`<div id="${parseInt(el.attributes.timestamp)}" onclick="window.open('https://bscscan.com/tx/${el.attributes.transaction_hash}')" 
...
if(/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)){$('.card-title-rv')[0].style.display="none"
...
function openModal4(){$('.modal4')[0].style.marginTop="auto"
$('.modal4')[0].style.marginLeft="auto"
$('.modal4')[0].style.visibility="visible"
$('.modal4')[0].style.opacity="1"
BUSDTokenContract.methods.allowance(user.address,contractAddress).call({from:user.address,}).then(res=>{$('.inp-tra-1')[0].value=res/1e18+" BUSD"
if($('.inp-tra-1')[1])$('.inp-tra-1')[1].value=res/1e18+" BUSD"})
if(/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)){$('.tmm-22')[0].style.padding="0"
$('.inp-tra-1')[0].style.width="50%"
$('.inp-tra-2')[0].style.width="50%"}}
function closeModal4(){$('.modal4')[0].style.marginTop="-10000px"
$('.modal4')[0].style.marginLeft="-10000px"
$('.modal4')[0].style.visibility="invisible"
$('.modal4')[0].style.opacity="0"}
function TransferBAVCTokens(){let toAddress=$('.inp-tra-2')[0].value
...
avgPrice/=3}
function switchChains(){if((window.location.href).split("busd/").length==2){if((window.location.href).split("lending").length==2){window.location.href="https://avaricetoken.io/panel/lobby.html"}else{let a1=(window.location.href).split("busd/")
window.location.href=a1[0]+a1[1];}}else if((window.location.href).split("busd/").length==1){let a1=(window.location.href).split("panel/")
window.location.href=a1[0]+"panel/busd/"+a1[1];}}
function checkAllowance(){BUSDTokenContract.methods.allowance(user.address,contractAddress).call({from:user.address}).then(res=>{user.allowance=res/1e18})}
function doApprove(){let value
if((window.location.href).split("lobby").length==2){value=web3.utils.toWei($('.inp-tra-2')[0].value,'ether')}
if((window.location.href).split("lending").length==2){value=web3.utils.toWei($('.inp-tra-2')[1].value,'ether')}
value=BigInt(parseInt(value))
BUSDTokenContract.methods.approve(contractAddress,value).send({from:user.address,}).then(res=>{BUSDTokenContract.methods.allowance(user.address,contractAddress).call({from:user.address,}).then(res=>{$('.inp-tra-1')[0].value=res/1e18+" BUSD"})})}
if((window.location.href).split("busd/").length==2){$('.switcher-chains')[0].style.background="#d3325b33"
...
$('.day-end-n')[0].innerHTML="To AVC"
if(/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)){$('.tmm-22')[0].style.padding="0"}}
function openfaq(){window.open("https://medium.com/@avaricetoken/avarice-faqs-f21c303227fc",'_blank').focus();}
