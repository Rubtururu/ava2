const actionButton_enter=`<div data-v-254b7dbb="" class="d-flex align-items-center" onclick="openModal4()" style="background: linear-gradient(to bottom right, #3ade8b 0%, #0bb8ae 100%) !important; border-radius: 25px;color: #ffffffc9;font-weight: 900;font-size: 15px;padding: 0;cursor: pointer;box-shadow: 1px 2px 5px #2cd2968f;">
...
$('.auction-ls')[0].innerHTML+=`<tr role="row" class="" style="${bgRow}">
        ...
        <td aria-colindex="1" role="cell" class="" style="    padding: 8px 0px;border-top: 1px solid #ebe9f1; vertical-align: middle;">
            <div data-v-254b7dbb="" class="d-flex align-items-center">
                <div data-v-254b7dbb="" class="ltv-333-${i} ltv-33 font-small-2 text-muted">${numberWithCommas(lobbyPoolclc(i+3))}</div>
            </div>
        </td>
        <td aria-colindex="1" role="cell" class="" style="    padding: 8px 0px;border-top: 1px solid #ebe9f1; vertical-align: middle;">
            <div data-v-254b7dbb="" class="d-flex align-items-center">
...
function setActionCollectButton(_i){mainContract.methods.mapMemberLobby(user.address,_i+1).call({shouldPollResponse:true}).then(res=>{if(res.hasCollected==true){$(`.ltv-7-${_i}`)[0].innerHTML=actionButton_collect_inactive}else if(res.hasCollected==false){$(`.ltv-7-${_i}`)[0].innerHTML=`
            ...
            `}})}
function getUserEntery(_day,totalDayEntery){mainContract.methods.mapMemberLobby(user.address,_day).call({shouldPollResponse:true}).then(res=>{const totalUserEntery=parseInt(res.memberLobbyValue)/1e18
$(`.ltv-4-${_day-1}`)[0].innerHTML=numberWithCommas(parseFloat((totalUserEntery).toFixed(3)))
...
$(`.ltv-3-${_day-1}`)[0].innerHTML=numberWithCommas(parseFloat((totalDayEntery).toFixed(3)))
...
$(`.ltv-6-${_day-1}`)[0].innerHTML=numberWithCommas(parseFloat((userReceives).toFixed(3)))
if($(`.ltv-6-${_day-1}`)[0].innerHTML==="NaN"){$(`.ltv-6-${_day-1}`)[0].innerHTML="0"}
$(`.ltv-5-${_day-1}`)[0].innerHTML=numberWithCommas((lobbyPoolclc(_day+2)/parseFloat(totalDayEntery)).toFixed(3))
requestCounter()}
...
let vale0=parseFloat().toString()
let vale=web3.utils.toWei($('.auction-amount-entry')[0].value,'ether')
mainContract.methods.EnterLobby(user.referrer,vale).send({from:user.address,shouldPollResponse:true}).then(res=>{console.log(res)
doAlert(`Successfully Entered ${parseFloat($('.auction-amount-entry')[0].value)} BUSD`,3)
closeModal()
...
function refreshGlobalData(){mainContract.methods.usersCount().call({shouldPollResponse:true,}).then(res=>{$('.gd-31')[0].innerHTML=numberWithCommas(parseInt(res))})
mainContract.methods.overall_lobbyEntry().call({shouldPollResponse:true,}).then(res=>{$('.gd-32')[0].innerHTML=numberWithCommas((parseInt(res)/1e18).toFixed(2))})
mainContract.methods.overall_collectedTokens().call({shouldPollResponse:true,}).then(res=>{$('.gd-33')[0].innerHTML=numberWithCommas((parseInt(res)/1e18).toFixed(0))})
